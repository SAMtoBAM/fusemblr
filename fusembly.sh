...
##A wrapper for the fusarium assembly project = fusembly (like fusilli; the worst pasta invented)
...
## RUN TO CREATE: mamba create -n fusembly ratatosk bioconda::filtlong bioconda::flye bioconda::fastp nextpolish2 bioconda::seqkit

#conda activate fusembly
...
[[ $hifi == "" ]] && echo "Warning running without PacBio Hifi data; therefore skipping final polishing step"
...
Ratatosk correct -v -Q 90 -c ${threads} -G -s $( ls ${pair1path} ${pair2path} ) -l filtlong_ont/${prefix}.${readstats}.fq.gz -o ratatosk_ont/${prefix}.${readstats}.ratatosk
...
## run flye
flye --nano-corr ratatosk_ont/${prefix}.${readstats}.ratatosk.fq.gz -m ${minovl} --genome-size ${genomesize} --asm-coverage ${coverage} --threads ${threads} -o flye_assembly/
